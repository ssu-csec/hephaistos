 ...
 * @param {URL} url
 * @param {{collectors: import('./collectors/BaseCollector')[], log: function(...any):void, urlFilter: function(string, string):boolean, emulateMobile: boolean, emulateUserAgent: boolean, runInEveryFrame: function():void, maxLoadTimeMs: number, extraExecutionTimeMs: number, collectorFlags: Object.<string, string>, failureLog:object}} data
 *
...
async function getSiteData(context, url, {
    ...
    context.on('targetcreated', async target => {
        ...
        //console.log(simpleTarget) : other type이 무엇인지..?
    ...
    try {
        ...
        fs.writeFileSync(finalPath+'/webpage.html', html, 'utf-8');
    } catch (e) {
...
module.exports = async (url, options) => {
    ...
    try {
        ...
        // 입력 디렉토리와 출력 파일을 지정합니다.
        const inputDirectory = options.finalPath;
        const outputFile = options.finalPath+'/webpage.js';

        // 디렉토리 내의 모든 JavaScript 파일을 읽어들이는 함수
        function readJSFilesInDirectory(directory) {
            ...
            files.forEach((file) => {
                ...
                if (path.extname(filePath) === '.html') {
                    // HTML 파일 읽기
                    const html = fs.readFileSync(inputDirectory+'/webpage.html', 'utf-8');
        ...
        // 디렉토리 내의 JavaScript 파일들을 읽어서 하나의 파일로 합칩니다.
        const combinedContent = readJSFilesInDirectory(inputDirectory);
        // 결과를 출력 파일에 쓰기
