...
class RequestCollector extends BaseCollector {
    ...
    async addTarget({cdpClient}) {
        try{
            ...
            console.log("Error occured in RequestCollector")
        }
    ...
    handleRequest(data, cdp) {
        ...
        if(type == "Script") // download document
        {
            var name = this._finalPath + "/script" + this._scriptNum + ".js"
            try {
                req(url, function (error, response, body) {
                    body = "//"+ url + "\n" + body
                    try{
                        fs.writeFileSync(name,beautify(body,{indent_size: 2, space_in_empty_paren: true}), 'utf-8');
                    }catch(e){
                        fs.writeFileSync(name, body, 'utf-8');
                    }
                });   
            } catch(e) {
                console.log('Error occured while script requests')
            }
            this._scriptNum++;
        }
