...
function forEachLabelHash(hostname, maximumNumberOfLabels, cb) {
    var hash = 5381;
    var label = 1;
...
function suffixLookup(hostname, _a) {
    var allowIcannDomains = _a.allowIcannDomains, allowPrivateDomains = _a.allowPrivateDomains;
    var matchIndex = -1;
    var matchKind = 0;
    var matchLabels = 0;
    var index = 1;
    forEachLabelHash(hostname, packed[0], function (hash, labelStart, label) {
        var match = 0;
        ...
        if (allowPrivateDomains === true && match === 0) {
            match = binSearch(packed, hash, index + 1, index + packed[index] + 1)
                ? 2 | 4
                : 0;
        }
        ...
        if (match !== 0) {
            matchKind = match;
            matchLabels = (match & 16) !== 0 ? label + 1 : label;
    ...
    else if ((matchKind & 4) !== 0) {
        var nextDotIndex = hostname.indexOf('.', matchIndex);
        publicSuffix = hostname.slice(nextDotIndex + 1);
    }
    else if ((matchKind & 16) !== 0) {
        var previousDotIndex = hostname.lastIndexOf('.', matchIndex - 2);
        publicSuffix = hostname.slice(previousDotIndex === -1 ? 0 : previousDotIndex + 1);
    }
    else {
        publicSuffix = hostname.slice(matchIndex);
    }
    return {
        isIcann: (matchKind & 1) !== 0,
        isPrivate: (matchKind & 2) !== 0,
