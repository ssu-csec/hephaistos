...
const makeFolder = require('./helpers/makeFolder');
 ...
 */
async function crawlAndSaveData(urlString, dataCollectors, log, filterOutFirstParty, dataCallback, emulateMobile, proxyHost, antiBotDetection, executablePath, maxLoadTimeMs, extraExecutionTimeMs, collectorFlags, finalPath, failureLog) {
...
module.exports = async options => {
    ...
    async.eachOfLimit(options.urls, numberOfCrawlers, (urlItem, idx, callback) => {
        var url = urlItem.split("/")
        var uniqueUrl = createUniqueUrlName(url[2])
        var finalPath = path.resolve(options.initPath, uniqueUrl);
        makeFolder(options.initPath, uniqueUrl);
        ...
        const task = crawlAndSaveData.bind(null, urlString, dataCollectors, log, options.filterOutFirstParty, options.dataCallback, options.emulateMobile, options.proxyHost, (options.antiBotDetection !== false), executablePath, options.maxLoadTimeMs, options.extraExecutionTimeMs, options.collectorFlags, finalPath, options.failureLog);
        ...
        async.retry(MAX_NUMBER_OF_RETRIES, task, err => {
            if (err) {
                ...
                log(chalk.red('Crawl failed'), err.message, chalk.gray(err.stack));
                options.failureLog.push([url[2], err.message])
