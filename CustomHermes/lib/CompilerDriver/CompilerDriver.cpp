  ...
  if (!cl::CustomOptimize.empty()) {
    ...
    switch (cl::OptimizationLevel) {
      ...
      case cl::OptLevel::OMax:
        runFullOptimizationPasses(M, savePath); // Data Flow Analysis
        break;
      case cl::OptLevel::Ot:
...
CompileResult compileFromCommandLineOptions(std::string savePath) {
#if !defined(NDEBUG) || defined(LLVM_ENABLE_STATS)
  if (cl::PrintStats)
  ...
  if (cl::BytecodeMode) {
    ...
    return processSourceFiles(context, std::move(fileBufs), savePath);
  }
}
} // namespace driver
} // namespace hermes
